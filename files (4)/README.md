...
## Version 8 — same timetable every school day, icon cache fix

Upload all of: index.html, script.js, style.css, compass-icon.png and apple-touch-icon.png (both icon files are the same image). Keep FONT-LICENSE.txt.

Every weekday now shows exactly the same six lessons (10HRC4 Homeroom, 10MATB1 Mathematics x2, 10COM3 Commerce x2, 10SCI4 Science). The Friday work-experience entry and all "Cancelled" labels were removed. Weekends and `holidayRanges` dates still show no classes. The class codes are fixed as written, so changing the year level in the profile no longer changes them.

The home screen icon is now loaded from `compass-icon.png?v=8`, a new file name, so iOS can't reuse the old cached icon. The browser-tab icon uses the same logo. To see it on an iPhone: delete the existing home screen shortcut, reload the site in Safari (close the tab first if needed), then Share → Add to Home Screen again.
